...
use crate::stat::instance::{StatInstance, StatType};
...
impl StatSource for Limb {
    fn stats(&self) -> Vec<StatInstance> {
        ...
        let self_stats: Vec<StatInstance> = self.modifiers
            .iter()
            .map(|modifier| modifier.stats())
            .fold(vec![], |mut acc, mut stats| { acc.append(&mut stats); acc } );

        let child_stats: Vec<StatInstance> = self.child_limbs
            .iter()
            .map(|limb| limb.stats())
            .fold(vec![], |mut acc, mut stats| { acc.append(&mut stats); acc } );
        ...
        stat_map
            .iter()
            .map(|(stat_type, stat_value)| StatInstance { stat_type: stat_type.to_owned(), value: *stat_value } )
            .collect()
    }
}
...
#[cfg(test)]
mod tests {
    use crate::stat::instance::StatType;
    use super::*;
